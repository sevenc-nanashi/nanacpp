#include "features/vec_map.hpp"

#include <cassert>
#include <string>

int main() {
  let nums = Vec<int>{1, 2, 3, 4};
  auto sq = vec_map<int>(nums, [](int x) { return x * x; });
  assert((sq == Vec<int>{1, 4, 9, 16}));

  auto strs =
      vec_map<std::string>(nums, [](int x) { return std::to_string(x); });
  assert(strs.size() == nums.size());
  assert(strs.front() == "1");
  assert(strs.back() == "4");

  let empty = Vec<int>{};
  auto empty_res = vec_map<int>(empty, [](int x) { return x; });
  assert(empty_res.empty());

  int acc = 0;
  auto prefix = vec_map<int>(nums, [&](int x) {
    acc += x;
    return acc;
  });
  assert((prefix == Vec<int>{1, 3, 6, 10}));

  // in-place 変換
  auto inplace = nums;
  vec_map_inplace(inplace, [](int x) { return x * 2; });
  assert((inplace == Vec<int>{2, 4, 6, 8}));

  Vec<std::string> words{"abc", "de"};
  vec_map_inplace(words, [](const std::string &s) { return s + "!"; });
  assert((words == Vec<std::string>{"abc!", "de!"}));

  Vec<int> empty_vec;
  vec_map_inplace(empty_vec, [](int x) { return x + 1; });
  assert(empty_vec.empty());

  let set = std::set<int>{5, 10, 15};
  auto set_mapped = vec_map<int>(set, [](int x) { return x / 5; });
  assert((set_mapped == Vec<int>{1, 2, 3}));

  vec_map_inplace(set_mapped, [](int x) { return x + 10; });
  assert((set_mapped == Vec<int>{11, 12, 13}));

  return 0;
}
